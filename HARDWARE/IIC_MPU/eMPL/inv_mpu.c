...
uint8_t mpu_dmp_get_data(void)
{
	...
	short gyros[3], accel[3], sensors;
	unsigned char more;
	long quat[4]; 
	if(dmp_read_fifo(gyros, accel, quat, &sensor_timestamp, &sensors,&more))
		return 1;	
	...
	**/
	if(sensors&INV_WXYZ_QUAT) 
	{
		...
		q3 = quat[3] / q30; 
		//计算得到俯仰角/横滚角/航向角
		out_angle.pitch = asin(-2 * q1 * q3 + 2 * q0* q2)* Radian_to_Angle;	// pitch
		out_angle.roll  = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* Radian_to_Angle;	// roll
		out_angle.yaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * Radian_to_Angle;	//yaw
	}else return 2;
	return 0;
}
